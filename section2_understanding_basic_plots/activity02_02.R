...
migratoryZebras <- read.csv("~/Dropbox/Study/github/Building-Interactive-Graphs-with-ggplot2-and-Shiny/section2_understanding_basic_plots/migratingZebras.csv")
head(migratoryZebras)

# plotting with geom_path
p <- ggplot(migratoryZebras, aes(latitude, longitude, color = zebraID)) 
p + geom_path() + 
  ggtitle("Zebra tracks, with geom_path()")

# plotting with geom_line()
p + geom_line() + 
  ggtitle("Zebra tracks, with geom_line()")
